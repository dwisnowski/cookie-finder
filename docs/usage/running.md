...
**Cloudflare Tunnel:** after `make init-cloudflare-tunnel`, **Settings → Cloudflare Tunnel** toggles the free quick tunnel (`*.trycloudflare.com`) on/off (disable stays off across reboot). Header badge shows CF status; Connect lists the live URL.
